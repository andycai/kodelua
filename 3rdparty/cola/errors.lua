...
local type, error, string, tonumber, tostring, xpcall = 
	type, error, string, tonumber, tostring, xpcall
local map = require "coro.map"
...
function Throw(msg, code)
	if code then
		error(string.format("<<%08u>> - %s", tonumber(code), tostring(msg)), 2)
	else
		error(msg, 2)
	end
end